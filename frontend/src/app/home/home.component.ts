...
import {StringStatsService} from "../service/string-stats.service";
...
export class HomeComponent implements OnInit {
  ...
  constructor(
    ...
    private stringStatsService: StringStatsService
  ) { }
  ...
  makeStatsRequest(data:any) {
    this.stringStatsService.getStats(data.input).subscribe( res => {
      this.forgeResonseObj(this.statsResponse,res,data.apiText);
    });
  }
  ...
  reverse({input}){
    this.reverseResponse = {data:this.stringStatsService.reverse(input)};
  }
}
